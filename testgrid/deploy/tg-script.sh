...
# try wraps commands that might fail due to external internet factors
function try () {
    ${1} || {
        echo "FAILED: ${1}"
        exit 1
...
try "kubectl apply -f https://github.com/kubevirt/kubevirt/releases/download/v0.32.0/kubevirt-operator.yaml"
try "kubectl apply -f https://github.com/kubevirt/kubevirt/releases/download/v0.32.0/kubevirt-cr.yaml"
kubectl wait --timeout=180s --for=condition=Available -n kubevirt kv/kubevirt
kubectl create ns cdi
try "kubectl apply -f https://github.com/kubevirt/containerized-data-importer/releases/download/v1.24.0/cdi-operator.yaml"
try "kubectl apply -f https://github.com/kubevirt/containerized-data-importer/releases/download/v1.24.0/cdi-cr.yaml"

echo "Installing krew KubeVirt plugin"
export HOME=/root
curl https://krew.sh/virt | bash
[ -f /root/.krew/bin/kubectl-virt ] || exit 1
